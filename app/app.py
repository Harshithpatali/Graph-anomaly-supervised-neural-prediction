...
if page == "🏠 Overview":
...
elif page == "🎯 Supervised Fraud Classifier":
    st.title("Supervised Fraud Classifier")
    st.markdown(
        """
        Leakage-safe pipeline: chronological split → feature engineering inside CV →
        **Logistic Regression / XGBoost / LightGBM** → class-weight imbalance handling →
        validation threshold → **locked test**.
        """
    )

    @st.cache_resource(show_spinner="Loading supervised classifier…")
    def load_supervised():
        path = ROOT / "artifacts" / "fraud_classifier.joblib"
        meta_path = ROOT / "artifacts" / "fraud_classifier_metadata.json"
        if not path.exists():
            return None, None
        from supervised_fraud import load_artifact
        import json as _json
        art = load_artifact(path)
        meta = _json.loads(meta_path.read_text()) if meta_path.exists() else art.get("metadata", {})
        return art["pipeline"], meta

    pipe, meta = load_supervised()
    if pipe is None:
        st.error("Artifact missing: `artifacts/fraud_classifier.joblib`. Train notebook 13 first.")
        st.stop()

    tm = meta.get("test_metrics", {})
    thr = float(meta.get("default_threshold") or meta.get("validation_threshold_f1") or meta.get("validation_threshold") or 0.5)
    maj = tm.get("majority_baseline_accuracy", 0.95)

    st.info(
        f"**Do not use accuracy alone.** Majority baseline (always “normal”) ≈ **{maj:.1%}**. "
        f"Model accuracy ≈ **{tm.get('accuracy', float('nan')):.1%}**. "
        "Judge ranking quality: PR-AUC, Precision@k, enrichment."
    )

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Selected model", str(meta.get("model_name", "?")))
    c2.metric("Test PR-AUC", f"{tm.get('pr_auc', float('nan')):.4f}")
    c3.metric("Test ROC-AUC", f"{tm.get('roc_auc', float('nan')):.4f}")
    c4.metric("Precision@k", f"{meta.get('precision_at_k_test', float('nan')):.3f}")

    c5, c6, c7, c8 = st.columns(4)
    c5.metric("Enrichment@k", f"{(meta.get('precision_at_k_test') or 0) / max(tm.get('base_rate') or 0.046, 1e-6):.1f}×")
    c6.metric("Recall", f"{tm.get('recall', float('nan')):.3f}")
    c7.metric("Precision", f"{tm.get('precision', float('nan')):.3f}")
    c8.metric("F1", f"{tm.get('f1', float('nan')):.3f}")
    ...
    if meta.get("operating_points_test"):
        st.subheader("Operating points on locked test")
        st.dataframe(pd.DataFrame(meta["operating_points_test"]), use_container_width=True)

    st.subheader("Score one raw transaction")
    with st.form("supervised_score_form"):
        ...
        with col_c:
            ...
            ip_address = st.number_input("ip_address", value=7.327584e8, format="%.0f")
    ...
    if submitted:
        row = pd.DataFrame([{
            "user_id": int(user_id), "signup_time": signup_time, "purchase_time": purchase_time,
            "purchase_value": float(purchase_value), "device_id": device_id, "source": source,
            "browser": browser, "sex": sex, "age": int(age), "ip_address": float(ip_address),
        }])
        try:
            proba = float(pipe.predict_proba(row)[0, 1])
            pred = int(proba >= thr)
            st.success(
                f"Fraud probability **{proba:.4f}** → "
                f"**{'FLAG for review' if pred else 'low risk'}** (threshold {thr:.3f})"
            )
